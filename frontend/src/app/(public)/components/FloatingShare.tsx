...
export const FloatingShare: React.FC = () => {
  // Estado para armazenar a URL atual (somente após o carregamento no cliente)
  const [urlToShare, setUrlToShare] = useState('');
  const textToShare = encodeURIComponent('Confira este site incrível!');

  useEffect(() => {
    // Executa apenas no cliente, evitando erro no SSR
    if (typeof window !== 'undefined') {
      setUrlToShare(encodeURIComponent(window.location.href || 'https://www.urlcurt.com.br'));
    }
  }, []);
  ...
  const networks = [
    ...
    {
      name: 'Instagram',
      shareUrl: `https://www.instagram.com/`, // Instagram não tem compartilhamento direto
      icon: (
